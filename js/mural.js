...
function mostrarAporte({ _id, contenido, usuario: autor, respuestas = [] }) {
  ...
  // Contenido
  const p = document.createElement('p'); p.textContent = contenido; card.appendChild(p);
  // Acciones
  const actions = document.createElement('div'); actions.className = 'mural-actions';
  const btnResponder = document.createElement('button'); btnResponder.textContent = 'Responder';
  btnResponder.addEventListener('click', () => abrirFormularioComentario(_id)); actions.appendChild(btnResponder);
  if (esAutor) {
    const btnCerrar = document.createElement('button'); btnCerrar.textContent = 'Cerrar aporte';
    btnCerrar.addEventListener('click', () => cerrarAporte(_id, card)); actions.appendChild(btnCerrar);
  }
  card.appendChild(actions);
  // Lista de comentarios
  const lista = document.createElement('div'); lista.className = 'comentarios-lista';
  respuestas.forEach(r => {
    const d = document.createElement('div'); d.className = 'comentario';
    d.innerHTML = `<strong>${r.autor}</strong>: ${r.contenido}`;
    lista.appendChild(d);
  });
  card.appendChild(lista);
  // Formulario de comentario
  const form = document.createElement('div'); form.className = 'form-comentario hidden';
...
async function cargarAportes() {
  try {
    mural.innerHTML = ''; aportesMostrados.clear();
    const res = await fetch(`${API_BASE_URL}/api/mural/today`);
    if (!res.ok) throw new Error(`Error ${res.status}`);
    const datos = await res.json(); datos.forEach(mostrarAporte);
  } catch (err) { console.error('Error al cargar el mural:', err); }
}
...
async function comentarAporte(id, texto) {
  ...
  } catch { alert('No se pudo comentar.'); }
}
...
async function cerrarAporte(id, card) {
  ...
  try {
    ...
    }); if (res.ok) card.remove(); else throw new Error();
  } catch { alert('No se pudo cerrar el aporte.'); }
}

// ===== Modal =====
function cerrarMensaje() { const noMostrar = document.getElementById('noMostrarCheckbox')?.checked; if (noMostrar) localStorage.setItem('noMostrarModal','true'); document.getElementById('modalOverlay').style.display='none'; }
...
function togglePopovers() {
  ...
  btnFormulario?.addEventListener('click', e => { e.stopPropagation(); formPanel.classList.toggle('mostrar'); misPanel.classList.remove('mostrar'); });
  btnMisAportes?.addEventListener('click', e => { e.stopPropagation(); misPanel.classList.toggle('mostrar'); formPanel.classList.remove('mostrar'); });
  document.addEventListener('click', e => { if (!e.target.closest('.formulario') && !e.target.closest('#btnFormulario')) formPanel.classList.remove('mostrar'); if (!e.target.closest('#misAportes') && !e.target.closest('#btnMisAportes')) misPanel.classList.remove('mostrar'); });
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (localStorage.getItem('noMostrarModal')==='true') document.getElementById('modalOverlay').style.display='none';
  cargarAportes(); togglePopovers();
});
